...
client.veri = require("./src/veri.json");
client.sistem = require("./src/ayar.json");
client.modüller = {}; 
client.altbaslik = client.veri.Tag + " " + client.veri.sunucuUfakIsim + " Developer by Catcher"
const qDb = require("quick.db");
const db = new qDb.table("ayarlar");
const cezaDb = new qDb.table("aCezalar");
const hook = new WebhookClient(client.veri.hosgeldinSistemi.webhookID, client.veri.hosgeldinSistemi.webhookTOKEN);
fs.readdir("./src/Komutlar", (err, files) => {
    if(err) return console.error(err);
    files = files.filter(file => file.endsWith(".js"));
    console.log(`[ ${files.length} ] Adet Register Komutları Yüklenecek!`);
    files.forEach(file => {
        let referans = require(`./src/Komutlar/${file}`);
  ...
  fs.readdir("./src/Etkinlikler/", (err, files) => {
    if (err) return console.myTime(err);
    files.forEach(fileName => {
      let referans = require("./src/Etkinlikler/" + fileName);
        referans.onLoad(client);
        client.on(referans.Etkinlik, referans.onRequest);
      console.log(
        `[~ Register ~ ] (${fileName}) isimli etkinlik yüklendi.`
      );
    });
  });
...
if(newMessage.author.bot || newMessage.channel.type === "dm") return;
 ...
 client.on("message", (message) => {
    ...
    if([".tag", "!tag"].includes(message.content.toLowerCase())){ 
      return message.channel.send("ryú #1987"); 
    }
...
client.on("guildMemberAdd", async member => {
          ...
          let veri = client.veri;
          let catcherveri = db.get("ayar") || {};
          ...
          if (veri.yasakTaglar && !veri.yasakTaglar.some(tag => member.user.username.includes(tag)) && yasakTaglilar.some(x => x.includes(member.id))) await cezaDb.set('yasakTaglilar', yasakTaglilar.filter(x => !x.includes(member.id)));
           if (veri.yasakTaglar && veri.yasakTaglar.some(tag => member.user.username.includes(tag))) {
           if(src.Roller.yasakliTagRolu) member.roles.set([src.Roller.yasakliTagRolu]).catch();
      if (!yasakTaglilar.some(id => id.includes(member.id))) cezaDb.push('yasakTaglilar', `y${member.id}`);
      member.send(`**${src.Tag} ${src.sunucuIsmi}** Yasak tag'da bulunduğunuz için otomatik olarak cezalıya atıldınız tagı çıkartana kadar cezalıda kalmaya devam ediceksin.`).catch();
    } else if (guvenili) {
            if(src.Roller.supheliRolu) member.roles.set([src.Roller.supheliRolu]).catch();
            if(src.Kanallar.supheliLogKanali && member.guild.channels.cache.has(src.Kanallar.supheliLogKanali)) return;
          } else if(src.kayıtRolleri.kayıtsızRolleri) member.roles.add(src.kayıtRolleri.kayıtsızRolleri).catch();
          if(sürelisusturma.some(x => x.id === member.id) || kalicisusturulma.some(x => x.includes(member.id))) member.roles.add(src.Roller.muteRolu).catch();
          if(sessusturulma.some(x => x.id === member.id) && member.voice.channel) member.voice.setMute(true).catch();
          if(src.IkinciTag) member.setNickname(`${src.IkinciTag} İsim | Yaş`).catch();
          else if(src.Tag) member.setNickname(`${src.Tag} İsim | Yaş`).catch();
          ...
          hook.send(`
           \`>\` ${veri.sunucuUfakIsim}**1987s of ryú, Sunucusuna Hoşgeldin!**\n
          \`>\` ${member} (\`${member.id}\`) **hesabın __${tarih}__ tarihinde ${client.gecmisTarihHesaplama(member.user.createdAt)} oluşturulmuş.**\n
          \`>\` **Ailemiz seninle birlikte** \`${member.guild.memberCount}\` **kişiye ulaştı! tagımızı alarak bizlere destek olabilirsin, <@&824637934028062720> rolüne sahip yetkililer senin ile ilgilenecektir.**\n
          \`>\` **Sunucu kurallarımız <#${src.Kanallar.kurallarKanal}> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.**\n
  `); 
...
client.login(client.sistem.a_Token).catch(err => console.error("[~ Register ~] Discord API Botun tokenini doğrulayamadı."));
