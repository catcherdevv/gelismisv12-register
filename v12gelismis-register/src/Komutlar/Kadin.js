...
module.exports = {
    Isim: "kız",
    Komut: ["kız", "k"],
    Kullanim: "kadin @Catcher/ID <isim> <yaş>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.a_Prefix}kadın @Catcher/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!isim || !yaş) return message.channel.send(`Hata: Lütfen tüm argümanları doldurunuz!  __Örn:__  \`${client.sistem.a_Prefix}kadın @Catcher/ID isim yaş\``).then(sil => sil.delete({timeout: 5000}));
        ...
        uye.setNickname(`${BelirlenenIsim}`).catch();
            ...
            let kadın = uye.roles.cache.filter(x => x.managed).map(x => x.id).concat(veri.kadinRolleri)
      ...
      let isimsorgu = kullaniciverisi.get(`k.${uye.id}.isimler`) || [];
       ...
       IsimGecmisi = isimsorgu.length > 0 ? isimsorgu.map((value, index) => `\`${value.Isim}\``).join("\n") : "\n✅ Üyenin herhangi bir kayıtı bulunamadı.";
            message.channel.send(embed.setDescription(`${uye}, adlı üye başarıyla ${message.author}, tarafından **Kadın** olarak kayıt edildi.\n\n**❌ Bu Kullanıcının Veri Tabanındaki Geçmiş İsimleri [${Liste}]**\n${IsimGecmisi}`)).then(sil => sil.delete({timeout: 15000}));   
